...
public class MyCustomString implements MyCustomStringInterface{
	    ...
	    // method prints whitespaces as characters
	    public String getEveryNthCharacterFromBeginningOrEnd(int n, boolean startFromEnd) 
	    		throws IllegalArgumentException, NullPointerException {
	    	StringBuilder sb = new StringBuilder();
	    	if(n <= 0) throw new IllegalArgumentException("No chars found");
	    	
	     	if(word == null && n > 0) throw new NullPointerException("But how?");
	 
	    	// char position value greater than length of our String
	    	if(n > word.length())
	    		return sb.toString();

	    	if(startFromEnd == false) {
	    		for(int i=n-1; i<word.length(); i=i+n){
	    		    sb.append(word.charAt(i));
	    		  }
	    		
	    	}
	    	else if (startFromEnd == true) {
	    		for(int j = word.length()-n;j>= 0; j=j-n) {
	    ...
	    public void convertDigitsToNamesInSubstring(int startPosition, int endPosition) 
	    	...
	    	if(startPosition > endPosition) throw new IllegalArgumentException("Start > End");
	    	if(startPosition < 0 || endPosition < 0 || 
	    			startPosition > word.length() || endPosition > word.length())
	    	...
	    	if(startPosition <= endPosition && startPosition > 0 && endPosition > 0 && word == null)
	    			throw new NullPointerException ("null word or we move out of pointer pos");
